...
class DeepSVDD(object):
    ...
    def savePointsOutsideR(self, dataset: BaseADDataset, device: str = 'cuda', n_jobs_dataloader: int = 0):
        """Saves points outside of finalized sphere and saves to .csv file"""
        
        if self.trainer is None:
            self.trainer = DeepSVDDTrainer(self.objective, self.R, self.c, self.nu,
                                           device=device, n_jobs_dataloader=n_jobs_dataloader)
            
        self.trainer.outside_r(dataset, self.net)
