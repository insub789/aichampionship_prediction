...
class baemin_Dataset(TorchDataset):
    def __init__(self, root: str, normal_class=0): #normal_class는 어차피 main 돌릴 때 지정해버림.
        ...
        self.train_set = MyBaemin(root_dir=self.root, train=True)
        self.test_set = MyBaemin(root_dir=self.root, train=False)

class MyBaemin(Dataset):
    def __init__(self, root_dir, train, transform = None):
        train_dir = os.path.join(root_dir, 'train_data.csv')
        test_dir = os.path.join(root_dir, 'test_data.csv')
        
        temp = pd.read_csv(train_dir)
        temp = temp.iloc[:, 2:]
        temp = temp.fillna(0)
        normal_temp = self.normalize(temp)
        self.train_data = normal_temp.drop(['abuse_yn'], axis=1)
        self.train_labels = normal_temp['abuse_yn']
